...
+ vib → Vibrant Beacon，指基础等级加一级的活力信标，vib为简称
...
在所有轮次用完，或者自己主动结算后，你会被传送到挑战起点，也就是最终宝箱所在点。
...
游戏分为跑图阶段→战斗阶段→结算阶段的循环

跑图阶段中，你需要跑向信标所指示的地点，可以用位移技能，**也可以用坐骑**

战斗阶段的目标基本是战斗，完成后即可获得奖励。

结算阶段会给予当前信标的效果以及轮次宝箱的奖励
...
| <font color = "orange">**橙色信标**</font> | 青色信标和Vibrant祝福的是挑战轮数；在游戏开始时刷新权重较高，随着游戏的进行逐渐降低刷新权重；最多将信标选数扩充至6 |
...
除了自爆以外，辉光怪物还有概率使用小范围的爆炸技能，同样拥有巨大的伤害。

怪物所受到的辉光强度达到10及更高时，不再增长强度，而是会把辉光效果传播给周围未辉光的怪物，你可以理解为将辉光效果等效至概率上。
...
$$(\frac{n}{n+1})×100\%_{\text{(50\%,66.6\%,75\%...)}}$$
...
因此，衍生出一种清日常玩法：每个区域每天只跑4轮，或者跑满36pull(加上每日20pulls)，这样把每日奖励吃一轮不会显得浪费银牛又省时间。
