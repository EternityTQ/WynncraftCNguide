...
因此你不必堆砌过多的回转，专心堆砌伤害即可

如果可以的话，建议尽可能减少技能蓝耗，看你想打bash还是airshout+upper，适当获取一些减对应技能蓝耗的装备。

由于fallen的配装一般不会有很多的int，因此百分比减蓝耗会非常有用。

@tab Bmonk

bmonk需要大量的技能回转，以此来保证稳定的技能循环

除了水属性武器和有大量技能减耗的构筑以外，玩bmonk可能会有点难受。

由于bmonk的循环特性(bash会保持1cost)，如果你需要通过减少蓝耗来维持循环的话，可以不考虑1st的技能减耗。

除此之外，尽可能堆砌伤害。

@tab Paladin

paladin在当前版本中，并不算一个输出位。

有一个配装思路是尽可能获取hpr，然后点出左路的血契，使用血契来进行烧血输出，这样你就不用堆砌蓝耗了。

除此之外，paladin按照正常的配装构筑来就可以了。
...
bolt的技能循环非常变态，几乎每秒都得按满三个技能

因此其构筑中需要尽可能多的法力回转。

除此之外，还要尽可能减少技能蓝耗，比较建议带一件Xebec，能大幅减少配装压力。

类似Stratiformis这样的武器玩Bolt会非常尴尬，因为实在是缺少合适的维持回转手段。

bolt把蓝转空是很正常的事。

除此之外，也是尽可能堆砌伤害。

移速反而不需要太考虑，bolt树中自带大量移速获取手段。

@tab trapper

trapper的技能树中自带大量回转，因此你完全不必考虑回转问题。

可以尝试尽可能堆砌伤害。

@tab sharp

Sharp的回转需求介于二者之间，只需要一点回转即可轻松转起来

20mr左右即可满足需求，尽可能堆砌伤害即可。
