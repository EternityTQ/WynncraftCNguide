...
::: info 核心计算机制
设定你的总固定恢复量为 R（基础恢复 + 装备 Raw），百分比加成为 P（hpr%）：

- **当你处于回血状态（R ≥ 0）时：**
  **最终恢复量 = R * (1 + P)**
  *解读*：如果你的 P 过低（<-100%），乘数就会变成负数，导致你从回血变成持续扣血。这种情况在搭配 Warp 等重度扣减 hpr% 的装备时极易发生。

- **当你处于扣血状态（R < 0）时：**
  **最终恢复量 = R * max(0, 1 - P)**
  *解读*：正数的 P 会不断**抵消**你的扣血效果。当 P ≥ 100% 时，无论你的负数 Raw 有多高，最终扣血量都会被强制保底锁定为 0；相反，负数的 P 则会进一步加剧你的扣血效果。
:::
