...
简单来说，采集副职负责获取各种原料，而制作副职则使用这些原料，再配合素材制作装备和消耗品。
...
具体的工具兑换地点和花费可以查看后文“快速提升副职等级 → 采集”中的工具兑换表
...
:::tip 原料对应的制作物品等级
原料从1级、10级后开始，每10级会有一个新的原料等级，直至100级，此后每5级会有新原料，直至115级。
...
每一个制作副职都会对应两种原料，具体的基础配方可以通过后文的配方查询器查看。
...
中间六格用于摆放功能素材，在没有特定词条的时候，素材的摆放位置**不会对产物效果有任何影响**

在理解了这六个素材格的位置之后，再来看下面的`Ingredient Effectiveness`就会直观很多。
...
`Ingredient Effectiveness`词条，它的效果是影响工作台内指定格子的素材的效果
...
| 制作副职 | 可制作物品 | 原料品质主要影响什么 | 不添加素材时的基础效果 / 特殊说明 |
| --- | --- | --- | --- |
| 盔甲师(Armouring) | 头盔、胸甲 | 血量、耐久 | 除了血量外将不会有任何词条 |
| 裁缝(Tailoring) | 护腿、靴子 | 血量、耐久 | 除了血量外将不会有任何词条 |
| 锻造师(Weaponsmithing) | 长矛、匕首 | 攻击力、耐久 | 除了攻击力外将不会有任何词条 |
| 木匠(Woodworking) | 弓箭、法杖、巫杖 | 攻击力、耐久 | 除了攻击力外将不会有任何词条 |
| 珠宝家(Jeweling) | 戒指、手镯、项链 | 耐久 | 不会有任何词条 |
| 药剂师(Alchemism) | 药剂 | 持续时间 | 所得产物为自制治疗药水，且会占用治疗药水槽 |
| 卷轴师(Scribing) | 卷轴 | 持续时间 | 所得产物卷轴的效果为恢复少量血量 |
| 厨师(Cooking) | 食物 | 持续时间 | 所得产物效果为恢复少量血量 |

关于药水、卷轴和食物的特殊说明：
+ 药剂为消耗品，使用后会给自己带来增益效果
+ 卷轴为消耗品，使用后会给半径9.5格内的所有友方玩家施加对应的效果；
+ 食物为消耗品，使用后会给自己带来增益效果，持续时间一般比药剂更长，但是效果较差


### 如何制作

根据你想制作的产物，在左侧两格中按照对应比例放入原料，并在中间六格放入需要使用的素材。

![](/assets/img/profession.md-09-15-00-51-57.png)
...
如果使用了存在`Ingredient Effectiveness`词条的素材，记得根据前面的规则检查摆放位置。

在放置好原料和素材后，在√处确认制作等级，你不能制作高于当前制作副职等级的产物。

:::tip

左侧的原料处只有配方正确，才可开始制作，制作配方可以将鼠标移动至中间的勾处查看，也可以移步文末查看配方查询器。

注意，哪怕有prof speed bomb，你也需要摆放原配方才可开始制作，原料需求减半的效果会在扣除时生效。
...
穿戴装备时受伤、手持武器攻击，都会导致对应的自制物品流失耐久

耐久不会影响装备属性，但是当耐久归零时，所穿戴的装备将立即失效，需要维修后才可恢复属性。
...
关于Bombs可以看[这一章](/guide/vip.html)

如果你有Champion，你可以通过炸弹钟即时看到所有的prof线路。如果没有，你也可以前往交流群问问其他Champion有没有prof线路

此外，在拥有采集速度加成炸弹(psb)的线路中，制作原料消耗减半，可以节省资源！

一般每隔一段时间(约3个月或重大节日)还会开启为期约3天的双倍经验周末活动，有时甚至会有三倍经验周末。

在双倍经验期间，与pxp叠加时，你可以同时获得3倍的采集/制作经验加成
...
准备完成后，在有prof炸弹的线路，到对应的采集资源点进行采集即可

注意原则：截止110级前，等级每逢5的尾数将会解锁新的工具等级，每逢0的尾数将会解锁新的资源。

所以练级时，**等级每逢5的尾数一定要换采集工具，每逢0的尾数马上去新解锁的资源点**，这能让你大大提升采集效率。
...
| Lv.105 | Dernic | Lutho Gate | <CC>971 74 -628</CC> |
...
| Lv.105 | Dernic Seed | Void Valley Upper | <CC>1089 135 -1130</CC> |
...
| Lv.105 | Dernic | Olmic Mine | <CC>492 93 -525</CC> |
...
| Lv.105 | Dernic Fish | Toxic Hill Upper | <CC>786 148 -983</CC> |
...
也就是说，原料和素材的品质不仅会影响产物本身，也会影响制作所得到的经验值。
...
原料方面，建议可以用你肝采集时所攒下来的原料，这样就可以省去原料的成本了，当然可能只会省一点。

当然T3原料我也不建议你拿来练级，最好留着。
...
需要注意的是，原料和素材的星级提供的经验倍率是乘算关系，也就是说，你用T3原料T3素材能达到最佳的效果，但是不太建议拿T3素材配T1原料这样做，比较亏，至少也得T2原料。
...
同名不同数值的消耗品原则是：同消耗品的同属性取最高，正负属性能同时起效并叠加，不同消耗品的同词条能叠加。
