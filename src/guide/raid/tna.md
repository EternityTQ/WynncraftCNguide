...
![](/assets/img/raids/tna1-1.png)
首先，整个场景内随机分布有多个Void hole，正常手段无法击杀。

其次，玩家出生点面前(红色平台)会刷新一颗Berserk berry，长按右键来拾取，拾取的玩家将会获得失明，无法使用技能，攻速被锁定为Super Slow并获得可以攻击虚空洞的能力。
...
Void hole下方有粉色方块作为位置标记，trapper的技能Fire Creep会在地面上产生火焰，遮挡这些标记方块，请务必避免释放
:::
...
![](/assets/img/raids/tna2-1.png)
这一挑战中，玩家会处于一棵巨大的树下，左右两侧分别有两个Soul Shrub的刷怪点（总共有4个刷怪点），场地中心是一个祭坛，正前方为进入树洞的传送门。

<u>进入这一挑战时，所有玩家初始已经持有灵魂，可以直接提交</u>

场地四周的Soul Shrub在死亡时，会补充周围玩家的灵魂。

站在场地中央的祭坛持续五秒，可以提交灵魂，用于打开中央树洞的所有大门，可以帮助机制处理者进出树洞。

请注意，Soul Shrub全部死亡后，才会再次刷新。
...
当其出现在房间内时，玩家的屏幕中央会有非常显眼的文字提示

![无视左边的bombbell..](/assets/img/tna.md-02-22-01-18-24.png)



进入树中的玩家需要将其击杀，**并返回树洞外，将掉落物提交至祭坛两侧**

**树中的房间门同样需要树外的玩家提交灵魂来开启**

重复两次即可开启机关通往下一关。

你可以打开[这个链接](/guide/tnatree.html)，将其中的代码存入json文件中，然后将文件放入wynntils的lootrun路线，即可在游戏内使用树洞路线指引。

![](/assets/img/tna.md-02-22-01-39-38.png)

进洞玩家推荐具有一定移速且有足够的伤害，例如hero、warp、dz。~~但是dz最好小心点别直接被射死了~~
...
1.寻找并击杀五只微隐身的Shadowling(一个特殊的敌人，通体黑色且周身有紫色条状材质)

该敌人不会全部刷新，只有在击杀一只后才会刷新新的一只。

![就长这B样](/assets/img/tna.md-02-22-02-35-25.png)
...
![](/assets/img/tna.md-02-22-02-36-08.png)
:::


## Room3
### Buleshift Wilds - 收集挑战
玩家会被传送至一大片平原之上，面前会有一个Blue bulb，并且平原上会持续刷怪。
...
平原外围会随机刷新Bulb Keeper，且其出现时，所有玩家会在聊天栏得到提示**A Bulb Keeper has spawned!**，且会出现非常响亮的号角声。找到它，将其击杀即可获得Red bulb。

![](/assets/img/tna.md-02-22-01-26-10.png)

击杀Bulb Keeper后，玩家会获得Red bulb，此时玩家无法使用技能无法攻击，靠近Blue bulb即可使用Red bulb为其充能。

:::tip
号角声的声源为Bulb Keeper刷新点。

你可以使用听声辩位，例如原地转一圈，来判断号角声源自哪里。

Bulb Keeper的火焰喷射比较痛，注意别被喷死了。
...
右侧第四个miniboss支线，会有两只头部是虚空洞的怪物，<u>距离起点较远的那只携带虚空物质</u>。需注意，一旦玩家接触到二者当中的任意一只，都会被立刻传送至另一只面前，就像进入了虚空洞一样，且会受到大幅伤害。

场地正中央的虚空洞会不断出现小怪来袭击传送门，其中的蓝名怪会攻击大传送门，传送门只有五点血量，如果传送门碎了就会立刻失败。

::star::在拿到前四个虚空物质后，三个Void Hole会出现血条，将其分别击杀三次即可召唤最后的miniboss Despairing Crawler	，击杀之获得最后一个虚空物质。
...
> 🟨 弹幕
> greg会向四周发射四道短距离的十字形弹幕，重复3-4次，每次对目标玩家造成其最大生命值10%的伤害。

> 🟥 重砸
> greg会发出诡异的诅咒声，随后高高跃起并猛击地面，在落点处形成一个巨大的虚空洞。玩家距离greg越近，受到的伤害越多。greg血量越低，此技能的连击次数越多。

> 🟧 冲刺
> greg会咆哮一声，随后高速冲向玩家，沿途留下多个小型虚空之洞，越接近技能结束，虚空之洞越大。玩家距离greg越近，受到的伤害越多。greg血量越低，此技能的连击次数越多，生成的虚空之洞也越多。

> 🟨 召唤
> greg会召唤多个<mob>The Armless</mob>，该小怪接近玩家时会造成大量伤害。

> 🟨 炸弹
> greg会生成多个漂浮的漩涡炸弹，炸弹落地后会发出两次滴答声并爆炸，玩家距离炸弹越近，受到的伤害越多。greg血量越低，生成的漩涡炸弹越多。

> 🟧 头颅
> greg会发出凋零呼吸声并释放烟雾粒子，随后为每个存活的玩家生成四颗追踪头骨。这些头骨不受重力影响，每颗头骨会直线冲刺三次，每次冲刺造成伤害时，减少命中目标的<tip name="治疗效率">治疗效率</tip>。头骨在完成所有冲刺后消失。若有玩家持续处于地面5格以上，此技能会更频繁释放。

> 🟧 眼球
> greg会生成多个粉色眼球，这些眼球完全无敌，但会在一定时间后消失。眼球接近玩家会造成大量伤害。greg血量越低，生成的眼球数量越多。

> 🟧 锁定
> greg会在目标玩家头顶生成一个跟随其移动的虚空之洞。片刻后，一只手掌会从洞中伸出并试图抓住目标，随后消失。此技能不仅对目标玩家造成伤害，还会对范围内的其他玩家造成大量伤害。

> 🟨 虚空之流
> greg会喷出一道15格长的蓝色物质。玩家试图穿过时会持续受到伤害。

> 🟧 虚空之握
> greg会快速冲向目标，沿途留下紫色光环，对范围内的玩家造成大量伤害。某些时候greg会在释放此技能时进行突进。

> 🟥 The Watched
> 这是greg的标志性技能，在其血量降至66%和33%时触发。在此状态下，greg无法被攻击。每次发射激光时，所有玩家会被白色光束锁定两次，随后被一道火焰光束击中，引发爆炸。爆炸对8格内的玩家造成**25%最大生命值**的伤害，6格内的玩家额外受到**30%最大生命值**的伤害。此技能期间，虚空洞不会被清除，但技能结束后会立刻重置一次(也就是修地板)。该技能会重复6次。
...
The Watched不再在一定时间后结束。相应的，竞技场的四个角落会各出现一座方尖碑（obelisk）。只要至少有一座方尖碑存在，The Watched就会持续。玩家可以通过将greg的激光引到方尖碑上来摧毁方尖碑，每座方尖碑需要摧毁两次。

![](/assets/img/tna.md-02-22-01-31-51.png)

graid中，greg会多一个特殊技能，会召唤向一个方向缓慢移动的纵向虚空洞，玩家触碰到该虚空洞会产生与掉进地面上的虚空洞时相同的效果。

此外,graid中greg召唤的头颅会减少更多的治疗效率。

在graid中，greg破坏场地的技能会减少释放频率。
